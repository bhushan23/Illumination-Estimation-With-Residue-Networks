...
def predict_sfsnet(sfs_net_model, dl, train_epoch_num = 0,
    ...
    wandb.log({suffix+' Total loss': tloss/len_dl, 'Val Albedo loss': aloss/len_dl, 'Val Normal loss': nloss/len_dl, \
               'Val Recon loss': rloss/len_dl}, step=train_epoch_num)
            
    # return average loss over dataset
    return tloss / len_dl, nloss / len_dl, aloss / len_dl, rloss / len_dl
...
def train(sfs_net_model, syn_data, celeba_data=None, read_first=None,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for bix, data in enumerate(syn_train_dl):
            ...
            # Apply Mask on input image
            # face = applyMask(face, mask)
            predicted_normal, predicted_albedo, out_shading, out_recon = sfs_net_model(face)
        ...
        print('Epoch: {} - Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}'.format(epoch, tloss, \
                                                                                                    nloss, aloss, rloss))
        log_prefix = 'Syn Data'
        if celeba_data is not None:
            log_prefix = 'Mix Data '
        ...
        if epoch % 1 == 0:
            print('Training set results: Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}'.format(tloss / syn_train_len, \
                    nloss / syn_train_len, aloss / syn_train_len, rloss / syn_train_len))
            # Log training info
            wandb.log({log_prefix + 'Train Total loss': tloss/syn_train_len, log_prefix + 'Train Albedo loss': aloss/syn_train_len, log_prefix + 'Train Normal loss': nloss/syn_train_len, \
                       log_prefix + 'Train Recon loss': rloss/syn_train_len})
            ...
            v_total, v_normal, v_albedo, v_recon = predict_sfsnet(sfs_net_model, syn_val_dl, train_epoch_num=epoch, use_cuda=use_cuda,
            ...
            wandb.log({log_prefix + 'Val Total loss': v_total, log_prefix + 'Val Albedo loss': v_albedo, log_prefix + 'Val Normal loss': v_normal, \
                       log_prefix + 'Val Recon loss': v_recon})
            

            print('Val set results: Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}'.format(v_total,
                    v_normal, v_albedo, v_recon))
        ...
        if epoch % 5 == 0:
            ...
            print('Test-set results: Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}\n'.format(t_total,
                                                                                                    t_normal, t_albedo, t_recon))
...
def train_syn_celeba_both(sfs_net_model, syn_data, celeba_data,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        print('Epoch: {} - Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}, CelebA loss'.format(epoch, tloss, \
                    nloss, aloss, rloss, celeba_tloss))
        if epoch % 1 == 0:
            print('Training set results: Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}, CelebA Loss: {}'.format(tloss / syn_train_len, \
                    nloss / syn_train_len, aloss / syn_train_len, rloss / syn_train_len, celeba_tloss / celeba_train_len))
            # Log training info
            wandb.log({'Train Total loss': tloss/syn_train_len, 'Train Albedo loss': aloss/syn_train_len, 'Train Normal loss': nloss/syn_train_len, \
                        'Train Recon loss': rloss/syn_train_len, 'Train CelebA loss:': celeba_tloss/celeba_train_len}, step=epoch)
            ...
            print('Synthetic Val set results: Total Loss: {}, Normal Loss: {}, Albedo Loss: {}, Recon Loss: {}'.format(v_total,
                    v_normal, v_albedo, v_recon))
